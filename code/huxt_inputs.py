...
def map_v_inwards(v_orig, r_orig, lon_orig, r_new):
    """
    Function to map a single value of v from r_orig (in rs) to r_inner (in rs) 
    ...
    #test reduced form used in Recurrent-HUXt paper:
    #T_integral =  (rH / (v0 * (1 + alpha))) * np.log((1 + alpha) * np.exp((r_orig - r_new) / rH) - alpha)
...
def map_v_boundary_inwards(v_orig, r_orig, r_new, b_orig=np.nan):
    ...
    Produces the required longitude shift and remaps the data series returned on input grid
